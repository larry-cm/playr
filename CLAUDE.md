...
- Guardas de `comprar()` (gasta plata real; ante la duda no paga): rol en el servidor · cantidad 1..`MAX_CANTIDAD`(10) · login en frío y producto/precio/stock/saldo **en vivo** (el precio que el manager confirmó debe ser el de ahora) · `iniciar_compra` (idempotencia por `request_id` + candado global: una sola compra `iniciada` a la vez) · el carrito debe estar **vacío** (no se toca lo ajeno) · el checkout debe mostrar exactamente ese ítem/cantidad/total, pago `wallet`, saldo suficiente y **ningún campo obligatorio extra** · máx. 60 s antes de pagar. Rechazo explícito del proveedor ⇒ `fallida`; respuesta dudosa (timeout, HTML) ⇒ `incierta` y **nunca se reintenta** el pago. Un error posterior al pago jamás se presenta como "no se pagó".
- DB (migración `20260920190001_bodega.sql`): `business.compra_proveedor` (estados `iniciada|pagada|registrada|pendiente_registro|fallida|incierta`; nadie escribe directo, solo `iniciar_compra`/`actualizar_compra`/`registrar_licencias`, `security definer` que exigen admin/manager con `business.es_staff()`). `registrar_licencias` es atómica (todo o nada), cifra la clave (`pgp_sym_encrypt`, `ACCOUNT_ENC_KEY`), **no duplica** (clave natural plataforma+correo+perfil+vencimiento) y crea el `producto` que falte **sin `precio_venta`** (no sale en la Tienda hasta fijarlo en Productos; si estaba eliminado se revive también sin precio). `createProductoAction` usa la misma función (antes insertaba `profile.precio_venta`, columna que ya no existe, y no revisaba el error).
- Entrega: cada llave = texto libre `correo clave PERFIL n - PIN: x`. Los **combos** traen una credencial por plataforma (a veces en un solo texto con la marca `CRUNCHY`/`HBO` antes de la segunda, a veces en llaves separadas) y se reparten en una cuenta `pantalla` por plataforma según el orden del nombre; el costo del combo se divide en partes iguales. Lo que no se reconoce con certeza (sin perfil, formato raro, correo solo, plataformas que no cuadran) **no se registra**: queda `pendiente_registro` con motivo y aviso en la campana, la entrega sigue en el proveedor y el botón «Registrar» del historial la reintenta sin duplicar.
